        ...
        <div className="fixed inset-0 z-50 flex justify-center items-center bg-black bg-opacity-30">
            <div className="relative bg-white  rounded-lg px-[2rem] py-[2rem] font-montserrat">
                ...
                <div>
                    <div className="flex justify-around gap-[3rem] mb-[15%]">
                        <div >
                            <div className="flex flex-col bg-[#F4F4F4] rounded-lg p-[1rem] shadow-md w-[20rem]">
                                <div><p className="text-2xl mb-[5%]">Message:</p><FaQuoteLeft className="text-gray-300" /></div>
                    ...
                    <p className="absolute bottom-[2rem] left-[2rem] text-xl max-w-[40%]">Source: <span className="font-semibold text-gray-700">{selectedLead.source}</span> </p>

                    <div className="absolute bottom-[2rem] right-[2rem]">
                        <p className="text-xl">Status: <span className="font-semibold">{showStatusEdit || isLoading ? '' : colorizeStatus(selectedStatus)}</span></p>
                        {isLoading ? <div>Loading...</div> :
                            <div>
                                {!showStatusEdit && <button className="hover:text-blue-400 active:text-black text-sm" onClick={() => setShowStatusEdit(true)}>update</button>}
                                {showStatusEdit &&
                                    <div className="flex justify-center items-center gap-4">
                                        <select className="bg-gray-200 ring-2 ring-gray-700 p-1 hover:cursor-pointer shadow-lg" value={selectedStatus} onChange={(e) => setSelectedStatus(e.target.value)}>
                                            <option value="new">New</option>
                                            <option value="contacted">Contacted</option>
                                            <option value="converted">Converted</option>
                                        </select>
                                        <div className="flex flex-col text-lg">
                                            <button className="hover:text-red-300 active:text-black" onClick={() => setShowStatusEdit(false)}>cancel</button>
                                            <button className="hover:text-green-300 active:text-black" onClick={handleSaveStatus}>save</button>
                                        </div>
                                    </div>
                                }
                            </div>
